...
class PasswordController extends Controller
{
    ...
    public function loginOtpOnEmail(Request $request) {
        // dd($request);
        $login_id = !empty($request->user_id)?$request->user_id:NULL;
        $data['code'] = 200;
        if($login_id == NULL) {
    ...
    public function verify_otp_submit(Request $request) {
        ...
        if($otp == NULL) {
            ...
            $register_type = $users_data->register_type;
    ...
    public function verify_otp_submit2(Request $request) {
        // dd($request);
        $otp = !empty($request->otp)?$request->otp:NULL;
        $data['code'] = 200;
        if($otp == NULL) {
            ...
            if($otp==$original_otp) {
                ...
                // dd($host);
                        ...
                        $data['message']='User Created Successfully. Please login to continue.';
                    //if not then redirect to change password change
                return json_encode($data);
            }
            else {
    ...
    public function verify_token(Request $request){
        $email=!empty($request->user_id)?$request->user_id:'';
        $user_token=!empty($request->user_token)?$request->user_token:'';
        $user= User::where('email',$email)->first();
        if($user){
            if($user->remember_token==$user_token){
    ...
    public function verifyFirstOTP(Request $request){
        ...
        if(empty($otp) || empty($users_id)){
            dd('User Record OR OTP does not match');
        }
        else
        {
            $remember_token = DB::table('users')->where('id',$users_id)->first()->remember_token;
            $role_id = DB::table('users')->where('id',$users_id)->first()->role_id;
            if($otp == $remember_token){
                $update_query=DB::table('users')->where('id',$users_id)->update(['first_time_login' => 1]);
                if($update_query){
    ...
    public function updateCredentials(Request $request){
        ...
        $username = !empty($request->username)?$request->username:$current_username;
        $current_password = $request->current_password;
        $new_password = $request->new_password;
        $auth_data=DB::table('users')->where('id',$users_id)->first();
        if(empty($auth_data)){
            dd('User Record Not Found');
        }
        else
        {
            if (!Hash::check($current_password, $auth_data->password))
             {
                ...
                $update_query=DB::table('users')->where('id',$users_id)->update($update_arr);
                if($update_query){
                    Session::flash('message','Credentials Updated Successfully');
                    if($auth_data->role_id == 3){
