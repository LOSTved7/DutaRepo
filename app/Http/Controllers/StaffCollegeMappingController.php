...
class StaffCollegeMappingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($college_name) && $college_name =='All'){
            $data->whereIn('college_name', $mapped_college_to_staff1);
        }
    ...
    public function get_mapped_college(Request $request)
    {
       $gat_nayak_id = !empty($request->gat_nayak_id)?$request->gat_nayak_id:'';
       $mapped_college_to_staff = DB::table('gat_nayak_college_mapping')
                     ->where('staff_profile_id',$gat_nayak_id)
                     ->where('status',1) 
                     ->pluck('college_name');
        return response(json_encode($mapped_college_to_staff));
    }
    ...
    public function send_whatsapp_notification_bulk(Request $request)
    {
        $subject = !empty($request->subject) ? $request->subject : '';
        $body = !empty($request->body) ? $request->body : '';
        $numbers = !empty($request->numbers) ? $request->numbers : '';
        $body = str_replace('    ', ' ', $request->input('body'));
        $staffIds = $request->input('staff_ids');
        $image_url = $request->input('image_url');
        $numbers = !empty($request->numbers) ? explode(',',$request->numbers) : [];
        // dd($numbers);
        $whatsapp_nos = $numbers;
        if (empty($whatsapp_nos)) {
            return back()->with('error', 'No WhatsApp numbers selected to send notification.');
        }
        foreach ($whatsapp_nos as $num) {
