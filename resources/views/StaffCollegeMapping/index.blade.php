                ...
                <div class="card-body">
                    ...
                    <form action="" method="GET">
                        ...
                        <div class="row">
                                ...
                                <select class="form-select single-select-clear-field" name="candidate_name" id="candidate_name" onchange="get_mapped_college(this.value);"
                                   data-placeholder="Select Name">
                                    ...
                                    @foreach($staff_Profile_arr as $key => $value)
                                        <option value="{{$key}}" {{ old('candidate_name', request('candidate_name')) == $key ? 'selected' : '' }}>
                                            {{ $value}}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                <label for="college_name" class="form-label">College</label>
                                <select class="form-select single-select-clear-field" name="college_name" id="college_name"
                                   data-placeholder="Select College">
                                    <option></option>
                                   <option value="All" {{ old('college_name', request('college_name')) == 'All' ? 'selected' : '' }}>All</option>
                                    @foreach($mapped_college_to_staff1 as $key => $value)
                                        <option value="{{$value}}" {{ old('college_name', request('college_name')) == $value ? 'selected' : '' }}>
                                            {{ $duColleges_mast[$value]}}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="col-md-12 d-none mt-4" id="smsCardSection">
                            <div class="card">
                        ...
                        <div class="col-md-12 mt-4 d-none" id="mailCardSection">
                            <div class="card">
                                <div class="card-header">
                                    <h5>
                                        ...
                                        @if(!$data->isEmpty())
                                            ...
                                            @foreach($data as $key => $item)
                                                @php
                                                    ...
                                                    $full_name = !empty($staffProfiles[$item->id]) ? $staffProfiles[$item->id] : '';
                                                @endphp
                                                <tr>
                                                    <td>{{ $i++ }}</td>
                                                    <td><input type="checkbox" class="staffCheckbox" name="staff_ids[]"
                                                            value="{{ !empty($item->id)?$item->id:'' }}"></td>
                                                    <td>{{ $full_name }}</td>
...
@section('js')
    <script>
        ...
        function get_mapped_college(value) {
            var gat_nayak_id = value;
            const duColleges_mast = @json($duColleges_mast);
            if (gat_nayak_id) {
                $.ajax({
                    url: "{{ route('get_mapped_college') }}",
                    type: 'POST',
                    data: {
                    ...
                    success: function (response) {
                        ...
                        $('#college_name').append('<option value="All">All</option>');
                        $.each(data, function (key, val) {
                            $('#college_name').append('<option value="' + val + '">' + duColleges_mast[val] || '' + '</option>');
                        });
                        }
                    },
                    error: function (xhr) {
                        console.error("Error fetching mapped colleges:", xhr);
                    }
                });
            } else {
                $('#college_name').empty();
